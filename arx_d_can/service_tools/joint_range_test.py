...
def select_joint_config(
    ...
    if transform_path is not None:
        # Corina 的 J1～J4 是独立电机。若精简的行程测试配置不包含机型定义的踝关节轴，
        # 则不能加载与该精简关节列表不匹配的 J5/J6 变换。
        coupled_candidates = {
            name
            for name in by_name
...
def execute_move(
    ...
    *,
    ...
    for point in points:
        ...
        if now >= next_report:
            state = arm.read_cached_state().arm
            print(
