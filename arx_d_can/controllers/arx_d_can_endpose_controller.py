"""ArxDCanEndPose — 末端位姿控制器（IK + 轨迹规划）。
...
控制循环中按组发送：arx_d_can.arm.send_pos_vel() → arx_d_can.gripper.send_mit()
（posvel 模式），或 arx_d_can.arm.send_mit() → arx_d_can.gripper.send_mit()（mit 模式）。

使用示例::
----
    from teleop.adapters.arm.arx_d_can.controllers import ArxDCanEndPose

    arx_d_can = ArxDCan()

    # POS_VEL 模式（默认）
    ctrl = ArxDCanEndPose(arx_d_can, arm_control_mode="posvel")
    ctrl.start()
...
----
    ...
    # MIT 模式
    ctrl_mit = ArxDCanEndPose(arx_d_can, arm_control_mode="mit")
    ctrl_mit.start()
...
上下文管理器::
    ...
    with ArxDCanEndPose(arx_d_can, arm_control_mode="mit") as ctrl:
        ctrl.move_to_ik(x=0.3, y=0.0, z=0.3)
"""
...
class ArxDCanEndPose:
    ...
    def __init__(
        ...
        self.arx_d_can = arx_d_can
        self._arm_group = arx_d_can.groups.get("arm", None)
        self._gripper_group = arx_d_can.groups.get("gripper", None)
        self._has_gripper = arx_d_can.has_gripper
    ...
    def start(self) -> None:
        ...
        self.arx_d_can.start_control_loop(self._loop_cb)
        self._running = True
    ...
    def end(self) -> None:
        if not self._running:
            return
        self.safe_home()
        self.arx_d_can.disconnect()
        self._running = False

    def __enter__(self) -> "ArxDCanEndPose":
        return self
    ...
    def safe_home(
        ...
        q_curr, _, _ = self.arx_d_can.get_state()
        ...
        while time.monotonic() < settle_deadline:
            q_now, _, _ = self.arx_d_can.get_state()
    ...
    def move_to_ik(
        ...
        if not result.success:
            print(f"[ArxDCanEndPose/IK] IK 未收敛  err={result.error:.3e}")
            return False
    ...
    def move_to_traj(
        ...
        q_start, _, _ = self.arx_d_can.get_state()
        ...
        if not ik_result.success:
            print(f"[ArxDCanEndPose/Traj] IK 失败  err={ik_result.error:.4f}")
            return False
        ...
        if not joint_traj:
            print("[ArxDCanEndPose/Traj] 轨迹为空")
            return False
    ...
    def _loop_cb(self, _: ArxDCan, dt: float) -> None:
        if self._arm_group:
            if self._arm_control_mode == "mit":
