...
def _packaged_runtime_path(runtime: DmDeviceRuntime) -> Path:
    matches = distribution_payload_candidates(
        "lib", "dm_device", runtime.version, runtime.lib_name
    )
    return matches[0] if matches else Path("__motor_drive_layer_payload_not_installed__")
...
def _install_hint(runtimes: list[DmDeviceRuntime]) -> str:
    ...
    return (
        ...
        f"{source_line}\n"
        "- Or run: python -m arx_d_can._motor_abi.dm_device_runtime --download\n"
