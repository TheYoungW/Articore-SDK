...
def test_dual_mit_send_forwards_each_side_parameters() -> None:
    ...
    class Runtime:
        @property
        def health(self):
            raise AssertionError("raw MIT success path must not read runtime health")
...
def test_public_dual_raw_mit_hot_path_does_not_read_feedback_or_scale_gains() -> None:
    robot = ArxDCanDualArm(
        control_mode="mit",
        left_gripper=False,
        right_gripper=False,
    ...
    def unexpected_feedback_read():
        raise AssertionError("raw MIT success path must not read SDK feedback")

    robot.left.read_cached_state = unexpected_feedback_read  # type: ignore[method-assign]
    robot.right.read_cached_state = unexpected_feedback_read  # type: ignore[method-assign]
    targets = (0.3,) * 7
    velocities = (0.2,) * 7
    feedforward = (100.0,) * 7
    ...
    robot.submit_raw_mit(
        ...
        left_velocities=velocities,
        right_velocities=velocities,
        kp=400.0,
        kd=4.0,
        left_feedforward_torques=feedforward,
        right_feedforward_torques=feedforward,
    )

    assert captured["left"] == targets
    assert captured["right"] == targets
    assert captured["left_velocities"] == velocities
    assert captured["right_velocities"] == velocities
    assert captured["left_torques"] == feedforward
    assert captured["right_torques"] == feedforward
    assert captured["left_mit_kp"] == 400.0
    assert captured["right_mit_kp"] == 400.0
    assert captured["left_mit_kd"] == 4.0
    assert captured["right_mit_kd"] == 4.0
