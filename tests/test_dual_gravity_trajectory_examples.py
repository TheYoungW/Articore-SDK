...
from arx_d_can.examples.control import (
    example_13_record_gravity_trajectory as record_example,
)
from arx_d_can.examples.control import (
    example_14_replay_trajectory as replay_example,
)
...
def test_record_parser_only_exposes_dual_product_inputs() -> None:
    args = record_example.build_parser().parse_args(
        ["--output", "dual.json", "--seconds", "12", "--hz", "500"]
    )

    assert args.seconds == 12.0
    assert args.hz == 500.0
    assert not hasattr(args, "arm_model")
    with pytest.raises(SystemExit):
        record_example.build_parser().parse_args(
            ["--output", "dual.json", "--hz", "500.01"]
        )
...
def test_replay_parser_exposes_only_normal_command_settings() -> None:
    ...
    assert not hasattr(args, "mit_kd")
    assert not hasattr(args, "mit_feedforward_torque")
...
def test_replay_does_not_duplicate_product_limit_logic_in_python() -> None:
    ...
    assert "move_joint_trajectory" not in source
