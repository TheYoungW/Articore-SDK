...
SafetyRuntime::SafetyRuntime(ArticoreRuntimeConfig config,
  ...
  if (!controller_group_ || !controllers_[0] || !controllers_[1]) {
    throw std::invalid_argument("Articore runtime requires one group and two controllers");
  }
  ...
  for (const auto& motor : motors) {
    ...
    has_left = has_left || motor.side == 0;
    has_right = has_right || motor.side == 1;
    MotorRecord record;
  ...
  if (!has_left || !has_right) {
    throw std::invalid_argument("Articore runtime requires motors on both sides");
  }
...
void SafetyRuntime::submit_pos_vel(const ArticorePosVelCommand* commands,
  ...
  bool should_fault = false;
  {
    ...
    if (api_.group_send_pos_vel(controller_group_, commands, count) != 0) {
      ...
      set_side_error_locked(0, error, true);
      set_side_error_locked(1, error, true);
...
void SafetyRuntime::report_feedback_failure(uint8_t side,
  ...
  if (side > 1) throw std::invalid_argument("feedback side must be 0 or 1");
...
bool SafetyRuntime::refresh_feedback_health(bool recovery_check,
  ...
  const bool has_unconfirmed = !unconfirmed.empty();
  {
    ...
    if (transports_ok && side_ok[0] && side_ok[1] && !has_motor_faults &&
        (!recovery_check || !has_unconfirmed)) {
      last_fresh_feedback_ = now - std::chrono::nanoseconds(
          std::max(maximum_age[0], maximum_age[1]));
      consecutive_feedback_failures_ = 0;
      return true;
    }
  }
  if (error.empty()) {
    if (!side_ok[0]) error = "CH0 feedback unhealthy: " + side_error[0];
    else if (!side_ok[1]) error = "CH1 feedback unhealthy: " + side_error[1];
...
void SafetyRuntime::recover() {
  {
    ...
    if (!sides_[0].connected || !sides_[1].connected) {
      throw std::runtime_error("both transports must be connected before recover");
    }
    hardware_transition_ = true;
  }
  std::string error;
